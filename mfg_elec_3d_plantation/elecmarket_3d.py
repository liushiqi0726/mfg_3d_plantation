...
class Conventional_3d(Agent):
    ...
    def __init__(self,name,cp,ap):
        ...
        self.Amint=ap['amint']
        self.Amaxt=ap['amaxt']
        self.Na=round(self.Abar/self.dt)+1
        ...
        V0=1.+delta*delta*self.X*self.dt/(self.dX*self.dX)
        V1 = -delta*delta*self.X*self.dt/(2*self.dX*self.dX)+kappa*(theta-self.X)*self.dt/(2*self.dX)
        V2 = -delta*delta*self.X*self.dt/(2*self.dX*self.dX)-kappa*(theta-self.X)*self.dt/(2*self.dX)
        alpha = (theta/stdIn)*(theta/stdIn)
        bet = theta/stdIn/stdIn
        ...
        for i in range(np.max([1,self.Amin]),self.Amax+1):
            indens[i]=ap['initial capacity']*bet*gamma.pdf(bet*self.X,alpha)/self.da/(self.Amax+1-np.max([1,self.Amin]))
        #for i in range(1,self.Na):
        #    indens[i] = ap['initial capacity']*bet*gamma.pdf(bet*self.X,alpha)/self.da/(self.Na-1)
    ...
    def F(self,x):
        ...
        return (x-self.epsilon>0)+x/self.epsilon*(x>0)*(x<=self.epsilon)
    def mbar(self,t):
        return np.sum(self.m_[t,1:,:],axis=0)*self.da+self.mtau_[t,:]
    def gain(self,p, cp, fp):
        return (convcoef*self.G(p-self.cFuel*fp[self.fuel]-self.X-self.cTax*cp) - self.rCost)
    def offer(self,p,cp, fp, t):
       # return sum(self.F(p-self.cFuel*fp[self.fuel]-self.X-self.cTax*cp)*self.mbar(t))*self.dX
        return sum(self.F(p-self.cFuel*fp[self.fuel]-self.X-self.cTax*cp)*(np.sum(self.m_[t,np.max([1,self.Amin]):self.Amax+1,:],axis=0)*self.dX*self.da+self.mtau_[t,:]*self.dX))
    def ioffer(self,p,cp,fp,t):
        #return sum(self.G(p-self.cFuel*fp[self.fuel]-self.X-self.cTax*cp)*self.mbar(t))*self.dX
        return sum(self.G(p-self.cFuel*fp[self.fuel]-self.X-self.cTax*cp)*(np.sum(self.m_[t,np.max([1,self.Amin]):self.Amax+1,:],axis=0)*self.dX*self.da+self.mtau_[t,:]*self.dX))
    ...
    def capacity(self):
        return np.sum(np.sum(self.m_[:,np.max([1,self.Amin]):self.Amax+1,:],axis=1),axis=1)*self.da*self.dX+np.sum(self.mtau_,axis=1)*self.dX
    def pot_capacity(self):
        return np.sum(self.mhat_,axis=1)*self.dX
    def exit_measure(self):
...
class Renewable(Agent):
    def __init__(self,name,cp,ap):
        ...
        indens =np.zeros((self.Na,self.NX))
        for i in range(np.max([1,self.Amin]),self.Amax+1):
            indens[i]=ap['initial capacity']*beta.pdf(self.X,alpha,bet)/self.da/(self.Amax+1-np.max([1,self.Amin]))
            self.preCalc(indens,indenshat,V,V0,V1,V2)
    def mbar(self,t):
        return np.sum(self.m_[t,1:,:],axis=0)*self.da+self.mtau_[t,:]
    def gain(self,p,cp,fp):
    ...
    def offer(self,t):
        return sum(self.X*(np.sum(self.m_[t,np.max([1,self.Amin]):self.Amax+1,:],axis=0)*self.dX*self.da+self.mtau_[t,:]*self.dX))
    def full_offer(self):
        # agent supply for given price level
        res = np.zeros(self.Nt)
        for i in range(self.Nt):
...
class Simulation:
    ...
    def __init__(self, cagents_2d,cagents_3d,ragents, cp):
        # agents: list of agent class instances
        # cp : common parameters
        self.cagents_2d = cagents_2d# conventional
        self.cagents_3d = cagents_3d# conventional
        self.cagents=np.concatenate([self.cagents_2d,self.cagents_3d])
